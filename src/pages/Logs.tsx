      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-lg bg-white/5 border border-white/10 text-[var(--text-primary)] placeholder-text-secondary focus:outline-none focus:border-blue-500/50 transition-colors"
          />
        </div>
        <select
          value={levelFilter}
          ...
          <div className="space-y-2">
            {logs.filter(log => pinnedLogs.includes(log.id)).map(log => (
              <div key={log.id} className="flex items-center gap-3 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                ...
                <span className="text-xs text-[var(--text-secondary)] font-mono">{log.timestamp}</span>
          ...
          {filteredLogs.map((log) => (
            ...
            >
              <button 
                ...
                className={`${log.pinned ? 'text-yellow-400' : 'text-[var(--text-secondary)] hover:text-yellow-400'}`}
              >
                <Pin className="w-4 h-4" />
              </button>
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${getLevelColor(log.level)}`}>
              ...
              <span className="text-xs text-[var(--text-secondary)] w-24">{log.module}</span>
              <span className="flex-1 text-sm text-[var(--text-primary)]">{log.message}</span>
